...
const EconomicSectorsClassification = () => {
  ...
  const allItems = [
    {
      id: 1,
      name: 'Farming',
      image: '/assets/farming.svg',
      answer: 'Primary Sector'
    },
    {
      id: 2,
      name: 'Fishing',
      image: '/assets/fishing.svg',
      answer: 'Primary Sector'
    },
    {
      id: 3,
      name: 'Textile',
      image: '/assets/textile.svg',
    ...
    },
    {
      id: 5,
      name: 'Manufacturing',
      image: '/assets/manufacturing.svg',
      answer: 'Secondary Sector'
    },
    {
      id: 6,
    ...
    },
    {
      id: 7,
      name: 'Mining',
      image: '/assets/mining.svg',
      answer: 'Primary Sector'
    },
    {
      id: 8,
      name: 'Teaching',
      image: '/assets/teaching.svg',
      answer: 'Tertiary Sector'
    },
    {
      id: 9,
  ...
  useEffect(() => {
    ...
    const allCorrectlyPlaced = allItems.every(item => {
      ...
      return itemAssignments[sector].some(
  ...
  useEffect(() => {
    const timeouts = Object.keys(sectorErrors)
      .map(sector => {
  ...
  const handleDrop = (e, targetSector) => {
    ...
    if (!isCorrectSector) {
      ...
      if (isNewPlacement) {
        ...
        setFeedback(
          `Oops! -${Math.abs(WRONG_PLACEMENT)} points. Try another sector.`
        )
      }
    ...
    sectors.forEach(sector => {
      if (newAssignments[sector].some(item => item.id === itemId)) {
        newAssignments[sector] = newAssignments[sector].filter(
          item => item.id !== itemId
        )
      }
    })
  ...
  useEffect(() => {
    if (unassignedItems.length === 0 && isCompleted) {
      setTimeout(() => {
        navigate('/level2')
      }, 5000)
    }
  }, [unassignedItems, isCompleted, navigate])
      ...
      <div className=' mx-auto'>
        <div className='flex justify-between items-center mb-4'>
          <h2 className='text-center text-xl font-bold'>
            Rearrange the given elements in appropriate system box
          </h2>
        ...
        {feedback && (
          <div
            className={`mb-4 p-3  rounded-lg text-center animate-pulse ${
              ...
              feedback.charAt(0) == 'G' || feedback.charAt(0) == 'C'
              ...
              <div
                className={`border-2 ${
                  sectorErrors[sector]
                    ...
                    : 'border-black'
                } rounded-md w-40 h-40 sm:w-48 sm:h-48 md:w-80 md:h-72 flex flex-wrap content-start justify-center p-2 overflow-y-auto transition-colors duration-300`}
