...
const Level2 = () => {
  ...
  const correctAnswers = {
    'Farming': { activity: 'Primary', conditions: 'Informal', ownership: 'Private' },
    'Fishing': { activity: 'Primary', conditions: 'Informal', ownership: 'Private' },
    'Police Officer': { activity: 'Tertiary', conditions: 'Formal', ownership: 'Public' },
    'Transportation': { activity: 'Tertiary', conditions: 'Formal', ownership: 'Mixed' },
    'Street Vendor': { activity: 'Tertiary', conditions: 'Informal', ownership: 'Private' }
  };
  ...
  const countCorrectAnswers = () => {
    ...
    Object.keys(correctAnswers).forEach(occupation => {
      Object.keys(correctAnswers[occupation]).forEach(field => {
        if (selections[occupation][field] === correctAnswers[occupation][field]) {
          count++;
        }
      });
    });
  ...
  return (
    <div className='bg-blue-50 h-screen w-screen flex justify-center items-center'>
      <div className='max-w-4xl mx-auto bg-white p-6 rounded-xl shadow-lg'>
        {feedback && (
          <div className={`mb-4 p-3 rounded-lg text-center animate-pulse ${
            isCompleted ? '' : 'fixed top-4 left-1/2 transform -translate-x-1/2 z-50 w-80'
          } ${
        ...
        <table className='w-full border-collapse bg-white rounded-xl overflow-hidden'>
          <thead>
            <tr>
              <th className='border border-gray-300 p-4 text-left'>Occupation</th>
              <th className='border border-gray-300 p-4 text-left'>Nature of Activity</th>
              <th className='border border-gray-300 p-4 text-left'>Employment conditions</th>
              <th className='border border-gray-300 p-4 text-left'>Ownership of Enterprises</th>
            </tr>
          </thead>
          <tbody>
              ...
              <tr key={occupation}>
                ...
                <td className='border border-gray-300 p-4'>
                  <select 
                    ...
                    onChange={(e) => handleChange(occupation, 'activity', e.target.value)}
                  >
                ...
                <td className='border border-gray-300 p-4'>
                  <select 
                    ...
                    onChange={(e) => handleChange(occupation, 'conditions', e.target.value)}
                  >
                    <option value="">Select Option</option>
                ...
                <td className='border border-gray-300 p-4'>
                  <select 
                    ...
                    onChange={(e) => handleChange(occupation, 'ownership', e.target.value)}
                  >
                    <option value="">Select Option</option>
        ...
        <div className='text-center mt-4 mb-8'>
          <p className='text-lg font-medium'>Select appropriate sectors from the dropdown</p>
        </div>
